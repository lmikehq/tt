...
function StaysMenu({ onDataChange }: FlightProps) {
  ...
  const removeRoom = () => {
    if (count.rooms > 1) {
      setCount((prev) => ({ ...prev, roomss: prev.rooms - 1 }));
    }
  };

  const addRoom = () => {
    setCount((prev) => ({ ...prev, rooms: prev.rooms + 1 }));
  };
  ...
  return (
    <FlightDropdown>
      ...
      >
        ...
        <Flex align="center" justify="space-between">
          <Text type="p" text="Rooms" weight={500} />
          <Flex gap=".75rem" align="center" justify="flex-end">
            <AiOutlineMinusCircle
              size={40}
              onClick={removeRoom}
              color={count.rooms <= 1 ? ttColors.gray : "#606060"}
            />
            <Text type="p" text={count.rooms.toString()} />
            <AiOutlinePlusCircle size={40} onClick={addRoom} />
          </Flex>
        </Flex>
      </Flex>
    </FlightDropdown>
