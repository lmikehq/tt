...
export const FieldInput = (props: FieldProps) => {
    ...
    const touched =
        getNestedValue(formik?.touched, name) || formik?.touched[name];
...
export const FieldPhone = (props: FieldProps) => {
    const { name, formik, placeholder, onChange, value, country, onChanged } =
        props;
    const touched = getNestedValue(formik?.touched, name);
    const error = getNestedValue(formik?.errors, name);
    ...
    return (
        <Section styles={{ position: "relative" }}>
            <PhoneInput
                ...
                inputProps={{ name }}
                inputStyle={{
                    border: touched && error ? `1px solid crimson` : "",
                }}
                onChange={handleChange}
                inputClass="w"
