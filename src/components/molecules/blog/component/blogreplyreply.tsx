    ...
    const handleLikeAndUnlikeComment = async () => {
        try {
            const response = await apiService(
                `/blog/${blogId}/comment/${comment._id}/reply/${reply._id}/reply/${replyreply._id}/like`,
                "POST"
            );
    ...
    const handleAddReplyReply = async () => {
        try {
            ...
            if (editInputValue !== "") {
                ...
                const response = await apiService(
                    `/blog/${blogId}/comment/${comment._id}/reply/${reply._id}/reply`,
                    "POST",
                    {
                        text: editInputValue,
                    }
                    ...
                    >
                        ...
                        <Flex align="center" color="#D9D9D9" gap="14px">
                            <Text
                                type="p"
                                text={
                                    replyreply.createdAt
                                        ? formatDistanceToNow(
                                              new Date(replyreply.createdAt),
                                              { addSuffix: true }
                                          )
                                        : ""
                                }
                ...
                >
                    ...
                    >
                        <BiSolidLike
                            ...
                            onClick={handleLikeAndUnlikeComment}
                        />

                        {replyreply?.likes?.length ? (
                            <p style={{ whiteSpace: "nowrap" }}>{`${
                                replyreply?.likes?.length
                            } ${
                                isMobile
                                    ? ""
                                    : replyreply?.likes?.length === 1
                                    ? "like"
                                    : "likes"
                            }`}</p>
                        ) : (
                    ...
                    >
                        ...
                        {replyreply?.replies?.length ? (
                            <p style={{ whiteSpace: "nowrap" }}>{`${
                                replyreply?.replies?.length
                            }    ${
                                isMobile
                                    ? ""
                                    : replyreply?.replies?.length === 1
                                    ? "comment"
                                    : "comments"
                            }`}</p>
                        ) : (
                    ...
                    <InputWrapper>
                        ...
                        {openEmoji && (
                            <EmojiPicker
                                ...
                                onSelect={handleEditSelectEmoji}
                            />
                        )}
                    </InputWrapper>
                )
