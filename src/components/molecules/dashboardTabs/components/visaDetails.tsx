...
function VisaDetail({
    ...
    visa: VisaResponseProp & OldVisaApplication;
    refetch: any;
}) {
    ...
    const [dependantPaymentInfo, setDependantPaymentInfo] = useState<{
        checkout_url: string;
        reference: string;
        price: number;
    }>({
    ...
    function getDependants() {
        if (isVisaApplication(visa)) {
            ...
            return [
                ...
                ...allParentDependants,
                ...allSiblingDependants,
            ];
        } else {
            const data: OldVisaApplication = visa as OldVisaApplication;
            const allImmediateFamilyDependants = data?.familyMembers.filter(
                (family) => family.accompanying === true
            );
                        ...
                        <img
                            src={
                                COUNTRY_FLAGS.find(
                                    (x) =>
                                        x?.code ===
                                        data?.primaryTraveller?.destination
                                            ?.code
                                )?.flag
                            }
                            alt="logo"
            ...
            return (
                <Flex direction="column" gap=".5rem">
                    <Text
                        ...
                        text={`${getLocationField("homeCountry")}(${
                            visa?.homeCountry?.code
    ...
    function getLocationField(field: string) {
        switch (field) {
            case "homeCountry":
                return (
                    visa?.primaryTraveller?.homeCountry?.name ||
                    visa?.homeCountry?.name ||
                    visa?.primaryTraveller?.homeCountry ||
                    "n/a"
                );
            case "destination":
                return (
                    visa?.primaryTraveller?.destination?.name ||
                    visa?.destination?.name ||
                    visa?.primaryTraveller?.destination ||
                    "n/a"
                );
            case "homeCode":
                return (
                    visa?.primaryTraveller?.homeCountry?.code ||
                    visa?.homeCountry?.code ||
                    " "
                );
            case "destCode":
    ...
    const dependantsFormik = useFormik({
        ...
        onSubmit: async (values, helpers) => {
            ...
            // console.log(values.dependants);
            apiService(
                `/visa/application/${visa._id}/add-accompanying`,
                "POST",
                values.dependants.map((dependant) => ({
                    membersEmail: dependant.memberEmail,
                    accompanying: true,
                    address: dependant.memberAddress,
                    gender: dependant.gender,
                ...
                .then((response) => {
                    ...
                    router.push(
                        `?reference=${response?.data?.data.reference}&checkout_url=${response?.data?.data?.checkout_url}`,
                        { scroll: false }
                    );
                })
                .catch((err) => {
        ...
        <>
            ...
            >
                ...
                <AccompanyPaymentModal
                    ...
                    open={
                        modalState.open &&
                        modalState.type === "dependant-payment"
                    }
                ...
                <AddVisaAccompanyModal
                    open={
                        modalState.open && modalState.type === "add-accompany"
                    }
                            ...
                            >
                                ...
                                <Flex gap="2rem" direction="column">
                                    ...
                                    <Flex>
                                        <Text
                                            type="p"
                                            text="Added Dependants"
                                            weight={600}
                                        />
                                    </Flex>
                                    <Section>
                                        {getDependants().length === 0 && (
                                            <Text
                                                type="p"
                                                text="Add Accompanies to see them here"
                                                ...
                                                {visa?.usedFormFeeVoucher && (
                                                    ...
                                                    >
                                                        ...
                                                        <Text
                                                            type="p"
                                                            text={
                                                                "Application fee paid with Travel Voucher"
                                                            }
                                                            size={"15px"}
                                                            weight={500}
                                                            transform="uppercase"
