...
export const groupFlightsByCombinationTripId = (
    flights: Flight[]
): Flight[][] => {
    const groupedFlights: { [key: string]: Flight[] } = {};

    flights.forEach((flight) => {
        const combinationTripId = flight.combination_trip_id;

        if (groupedFlights[combinationTripId]) {
            groupedFlights[combinationTripId].push(flight);
        } else {
            groupedFlights[combinationTripId] = [flight];
        }
    });
