...
interface ApplicationFormStoreActions {
  prevStep: () => void;
...
export const useApplicationFormStore = create<
  ApplicationFormStoreState & ApplicationFormStoreActions
>((set): ApplicationFormStoreState & ApplicationFormStoreActions => ({
  form: visaInitVals,
  step: 1,
  highestStep: 1,
  mode: Mode.init,
  ...
  nextStep: async ({ data }: { data: VisaFormUnionType }) => {
    ...
    set((state) => ({
      ...
      step: state.step + 1,
      highestStep:
        state.step + 1 > state.highestStep ? state.step + 1 : state.highestStep,
      form: {
        ...state.form,
        ...data,
      },
